-- Layer 1 team data foundation for FBref team stats and ELO ratings.
-- This migration adds canonical teams, alias normalization, raw team stats,
-- ratings snapshots, and convenience views for SQL consumers.
...
create table if not exists teams (
  ...
  source text not null default 'scoutedge-static',
...
create table if not exists matches (
  ...
  venue text not null,
  city text not null,
  home_team_slug text not null references teams(slug) on delete cascade,
  away_team_slug text not null references teams(slug) on delete cascade,
  source text not null default 'scoutedge-static',
  source_updated_at timestamptz,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
