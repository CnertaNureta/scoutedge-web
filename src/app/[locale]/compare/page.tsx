    ...
    <>
      ...
      <section className="relative py-24 md:py-36 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-[1440px] mx-auto text-center">
          <Badge variant="primary" size="md">{t('matchupsBadge', { count: totalMatchups })}</Badge>
      ...
      <section className="max-w-[1440px] mx-auto px-6 pb-20">
        {groups.map((group) => {
          ...
          return (
            <div key={group} className="mb-12">
              <SectionHeader className="mb-6">{t('groupHeader', { group })}</SectionHeader>
                  ...
                  <Link key={slug} href={`/compare/${slug}`}>
                    <GlassCard hover className="p-5">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="font-headline text-xs text-on-surface-variant mx-2 shrink-0">{t('vsLabel')}</span>
        ...
        <div className="mb-12">
          <SectionHeader className="mb-6">{t('sectionCrossGroup')}</SectionHeader>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
        ...
        <div className="mb-16">
          <SectionHeader className="mb-6">{t('sectionCompareAny')}</SectionHeader>
          <p className="text-on-surface-variant text-sm mb-6">
            {t('compareAnyDescription', { count: totalMatchups })}
          </p>
          <CompareSelector
            teams={teams.map((t) => ({
              slug: t.slug,
        ...
        <details className="mb-12 glass-panel rounded-2xl border border-white/[0.08] overflow-hidden">
          <summary className="px-6 py-5 cursor-pointer font-headline text-lg uppercase tracking-wide text-on-surface hover:text-primary transition-colors flex items-center justify-between">
            <span>{t('browseAllMatchups', { count: totalMatchups })}</span>
                ...
                teams.slice(i + 1).map((teamB) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-on-surface-variant text-xs mx-1.5 shrink-0">{t('vsLabelLower')}</span>
